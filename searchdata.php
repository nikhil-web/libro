<?php
$str  = mysqli_real_escape_string($db,$_POST['search']);
...
<?php
    echo '<h3 class="top_heading_products">'.$str.' Results </h3>';
?>
...
if($len>1){
   ...
   for ($x = 1; $x <= $len-1; $x++) {
    $snip[$x] = " OR book_name LIKE '$keyword[$x]' ";
   } 
    
      for ($x = 0; $x <= $len-1; $x++) {
    $snip2[$x] = " OR book_writer LIKE '$keyword[$x]' ";
    $snip3[$x] = " OR book_genre LIKE '$keyword[$x]' ";
   } 
    
$made_up_query= implode(" ",$snip);
$made_up_query2= implode(" ",$snip2);
$made_up_query3= implode(" ",$snip3);

    $sql = "SELECT * FROM library WHERE book_name LIKE '$keyword[0]' $made_up_query $made_up_query2 $made_up_query3 ORDER BY book_id ASC";

     }else{
         $sql = "SELECT * FROM library WHERE book_name LIKE '%$str%' OR book_writer LIKE '%$str%' OR book_genre LIKE '%$str%'  ORDER BY book_id ASC";
}
...
if (mysqli_num_rows($result) > 0) {
    ...
    echo '<h3 class="top_heading_empty_alert">No Matching Result</h3>';
}
